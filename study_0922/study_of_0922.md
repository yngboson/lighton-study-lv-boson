...
IMD의 신호가 끊긴 경우, 코일을 작동하던 전류가 없어지므로 코일이 열린다. 열린 코일은 외부에서 다시 붙이지 않을 때까지 IMD 신호가 통과하지 못함.
...
사진에서, BSPD 신호가 복구되어(5초 지점) HIGH로 돌아온 후 10초 지연되어 BSPD+10RESET 신호가 출력(10초 지점)되는 것을 확인할 수 있음.
...
대표적으로, Low-side 구조의 경우 1번 핀과 MOSFET 사이의 전선이 Ground와 단락되는 경우, 릴레이가 계속 열리게 되어 IMD에 문제가 발생하더라도 SDC가 차단되지 않을 수 있음. 따라서 추가적인 벤치마킹 및 스터디를 통해 기존 설계와 비교하여 Low-side 구조를 채택할지 결정해야 함.
